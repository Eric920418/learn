...
function EventBlock({ event }: { event: { sectionTitle: string; date: string; titleCn: string; titleEn: string; speaker: string; speakerTitle: string; location: string; info: string | null; } }) {
    ...
    <div>
      ...
      {event.info && (
        <div className="mt-10">
          ...
          <p className="text-sm leading-relaxed text-foreground/80">{event.info}</p>
        </div>
      )}
    </div>
  );
